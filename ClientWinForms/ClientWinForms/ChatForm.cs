...
namespace ClientWinForms
{
    public partial class ChatForm : Form
    {
        ...
        //запускає прослуховування повідомлення через функцію Task.Run(ReceiveAsync)
        public ChatForm(TcpClient client, NetworkStream stream, string username)
        {
            ...
            lstUser.Items.Add("user1");
            lstUser.Items.Add("user2");
            lstUser.Items.Add("user3");
            lstUser.Items.Add("user4");
            lstUser.Items.Add("user5");
            lstUser.Items.Add("user6");
        }
        ...
        private async void btnSendMessage_Click(object sender, EventArgs e)
        {
            ...
            string message = $"Me → {receiver}\n{text}";
            await SendAsync(message);


            AddChatHistory($"Me → {receiver}: {text}");
        ...
        private async Task ReceiveAsync()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    Invoke(() => AddChatHistory($"Received:{message}"));
                }
            }
            catch (Exception ex)
            {
        ...
        // ScrollToCaret - означає, що у текст-боксу є автоматична прокрутка до низу
