...
namespace ClientWinForms
{
    public partial class LoginForm : Form
    {
        ...
        private async void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (role == "user")
                {
                    ...
                    chat.Left = this.Left;  //ці два рядка дозволяють відкрити друге вікно WinForm
        ...
        //3. Метод ReceiveAsync. Читає з потоку у рідер та перетворює назад у текст
        private async Task<string> ReceiveAsync()
        {
            //byte[] buf = new byte[1024];
            //int bytes = await stream.ReadAsync(buf, 0, buf.Length);
